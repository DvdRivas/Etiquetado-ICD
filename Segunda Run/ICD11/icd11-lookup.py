...
icd11-lookup.py
---------------
REVERSE process of icd-experiment.py.

Reads the `icd11_code` column of the CSV and, for each code, queries the WHO
ICD-11 API for the official title of that entity. The result is written to
the `icd11_lookup` column.
...
PHASE 1 — Reverse lookup (deterministic, no LLM):
  1. NF-XXXX codes  -> resolved against nf_dictionary.json (they are not ICD-11)
  2. ICD-11 codes   -> WHO API /codeinfo -> stemId -> official title
  3. Fallback       -> WHO API /search restricted to the code
...
Everything runs in ENGLISH: titles are requested in English and the
consistency prompt is English, matching diagnosis_en, which is the column
the whole pipeline is built on. The diagnosis_es column is preserved as
source data but no step reads it. This also keeps the ICD11 and ICD10
folders symmetric — the ICD-10 API only exists in English.
...
Input/output CSV (same file, overwritten in place):
    filename | diagnosis_en | diagnosis_es | clinical_summary
            ...
            | hierarchical_distance | icd11_lookup | consistency
...
Iterative execution (dataset-run1.csv .. dataset-run10.csv):
  ...
  found next to it, reusing the same WHO client and loaded model across all
...
Usage:
    python icd11-lookup.py                      # runs all 10 runs
    python icd11-lookup.py --csv "dataset-run1.csv" --rows 5   # test
...
WHO credentials: WHO_CLIENT_ID and WHO_CLIENT_SECRET environment variables,
or via --who-client-id / --who-client-secret.
...
def read_csv_robust(path: str) -> tuple[list, list]:
    ...
    Returns (rows, fieldnames) preserving the original order.
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# 3. WHO ICD-11 API — REVERSE LOOKUP
# ─────────────────────────────────────────────────────────────────────────────

class WHOLookupClient:
    """WHO ICD-11 API client resolving code -> official title."""
    ...
    # Titles are always requested in English, matching diagnosis_en and the
    ...
    def authenticate(self) -> bool:
        if not self.client_id or not self.client_secret:
            print("[WHO] No credentials. Set WHO_CLIENT_ID and "
                  "WHO_CLIENT_SECRET or use --who-client-id/--who-client-secret\n")
            return False
        try:
            r = self.session.post(
                self.TOKEN_URL,
    ...
    def _extract_title(payload: dict) -> str:
        """The title arrives as {"@value": "..."} or as a plain string."""
    ...
    def _via_codeinfo(self, code: str) -> str:
        """
        /codeinfo/{code} returns the stemId (entity URI). That URI is then
        followed to obtain the official title.
        """
        ...
        # The stemId points at id.who.int; it is requested directly.
    ...
    def lookup(self, code: str) -> str:
        """
        Resolve an ICD-11 code to its official title.
        Returns "" when it could not be resolved.
        """
        code = (code or "").strip()
...
- The title is so unspecific that it carries no real classification.
  E.g. title "Certain specified conditions" or "Other disorders" -> no
...
async def run_consistency_phase(
    ...
    """
    Evaluate diagnosis vs icd11_lookup consistency on the given rows.
    ...
    for row in rows:
        ...
        # NF codes: there was no real ICD-11 labelling to audit. Their lookup
...
def resolve_row(row: dict, who: WHOLookupClient, nf_labels: dict) -> str:
    """
    Determine the icd11_lookup value for a row.
    ...
    # NF codes do not exist in ICD-11: they are resolved locally
...
async def run_for_csv(csv_path: str, who: WHOLookupClient, overwrite: bool,
    ...
    print(f"[CSV] {len(rows_in)} rows | {total_with_code} with icd11_code | "
    ...
    if not overwrite and total_with_code > len(pending):
        print(f"[CSV] {total_with_code - len(pending)} already had "
              f"icd11_lookup (use --overwrite to redo them)")
    print()
    ...
    if pending:
        ...
        print(f"  Resolved (WHO API)         : {stats['resolved']}    ({stats['resolved']/t*100:.1f}%)")
        ...
        if stats["unresolved"]:
            ...
            print("    of their own in ICD-11, or malformed codes.\n")
...
async def run_all(csv_paths: list[str], who: WHOLookupClient, overwrite: bool,
...
def main():
    parser = argparse.ArgumentParser(
        description="ICD-11 reverse lookup: code -> official title. Without "
    ...
    # No --lang: the whole pipeline runs in English, matching diagnosis_en.
    parser.add_argument("--workers", type=int, default=8,
                        help="Concurrent WHO API requests (default: 8)")
    ...
    who = WHOLookupClient(args.who_client_id, args.who_client_secret)
