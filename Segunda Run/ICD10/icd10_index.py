...
# The ICD-10 API and this XML are English-only, like the rest of the pipeline.
ICD10_LANG = "en"
...
def normalize(text: str) -> str:
    """
    Shared normalization for every comparison in the pipeline.

    Lowercase, apostrophes removed (so "Parkinson's", "Parkinson’s" and
    "Parkinsons" collapse), brackets turned into separators (so "[Takayasu]"
    tokenizes as "takayasu"), dashes turned into spaces, whitespace
    collapsed.
    """
...
def strip_dagger(code: str) -> str:
    """
    Drop the dagger/asterisk marker some sources append to a code.

    Orphanet writes 38 of its ICD-10 references as "A39.1+" or "B00.4*"
    (dagger/asterisk dual classification). The classification itself stores
    them as plain "A39.1", so without stripping the marker those references
    silently fail to match any catalogue entry.
    """
...
SEARCH_DEPTH = 3          # how many parent directories are walked
...
def find_data_file(filename: str, explicit: str | None = None,
    ...
    When an explicit path is given it is honoured and nothing else is
    searched. Otherwise the search walks from `base_dir` upwards (up to
    SEARCH_DEPTH levels) and, at each level, looks at the directory itself
    and then its direct subfolders — so a local copy next to the script wins
    over a shared copy in a sibling folder.
    """
    if explicit:
        path = Path(explicit).expanduser()
        return str(path) if path.is_file() else None

    for directory in [base_dir, *list(base_dir.parents)[:SEARCH_DEPTH]]:
